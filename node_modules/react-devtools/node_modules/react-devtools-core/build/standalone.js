module.exports = function(modules) {
    ...
    function onDisconnected() {
        ReactDOM.unmountComponentAtNode(node), node.innerHTML = '<h2 id="waiting">Waiting for a connection from React Native</h2>';
    }
    function onError(e) {
    ...
    function initialize(socket) {
        ...
        socket.onmessage = function(evt) {
            if ("attach:agent" !== evt.data) {
                var data = JSON.parse(evt.data);
                return data.$close || data.$error ? (console.log("Closing or Erroring"), onDisconnected(), 
                void (socket.onmessage = function(msg) {
                    "attach:agent" === msg.data && initialize(socket);
                })) : void (data.$open || listeners.forEach(function(fn) {
                    return fn(data);
                }));
            }
        }, wall = {
    ...
    function startServer() {
        ...
        server.on("connection", function(socket) {
            connected && (connected.close(), console.warn("only one connection allowed at a time"), 
            console.warn("closing the previous connection")), connected = socket, socket.onerror = function(err) {
                connected = !1, onDisconnected(), console.log("Error with websocket connection", err);
            }, socket.onclose = function() {
                connected = !1, onDisconnected(), console.log("Connection to RN closed");
            }, initialize(socket);
        }), server.on("error", function(e) {
            onError(e), console.error("Failed to start the DevTools server", e), restartTimeout = setTimeout(function() {
                return startServer(port);
            }, 1e3);
        });
        var embedFile = fs.readFileSync(path.join(__dirname, "../build/embed.js"));
        return httpServer.on("request", function(req, res) {
            res.end(embedFile);
        }), httpServer.on("error", function(e) {
            onError(e), document.getElementById("loading-status").innerText = "failed to start server :/", 
            ...
            close: function() {
                connected = !1, onDisconnected(), clearTimeout(restartTimeout), httpServer.close();
            }
        };
    }
    ...
    var Panel = __webpack_require__(5), React = __webpack_require__(6), ReactDOM = __webpack_require__(35), node = null, backendScript = fs.readFileSync(path.join(__dirname, "../build/backend.js")), wall = null, config = {
        reload: reload,
        alreadyFoundReact: !0,
        inject: function(done) {
            done(wall);
        }
    }, restartTimeout = null, DevtoolsUI = {
	...
	*/
    ...
    var _extends = Object.assign || function(target) {
        ...
        return _inherits(SearchPane, _React$Component), _createClass(SearchPane, [ {
            ...
            value: function(e) {
                if (191 === e.keyCode && "INPUT" !== e.target.nodeName && this.input && (this.input.focus(), 
    ...
    var _createClass = function() {
        ...
        function Breadcrumb() {
            return _classCallCheck(this, Breadcrumb), _possibleConstructorReturn(this, (Breadcrumb.__proto__ || Object.getPrototypeOf(Breadcrumb)).apply(this, arguments));
        }
